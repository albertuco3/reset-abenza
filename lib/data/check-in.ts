...
export async function getCheckInDefaults(
  ...
  const emomBy = Object.fromEntries(
    (emom ?? []).map((e) => [e.exercise, e]),
  );

  const sessionType =
    entry.session_type ?? defaultSessionForDate(entryDate);
  ...
  return {
    ...
    pull_up_kg: pullUp?.weight_kg != null ? Number(pullUp.weight_kg) : undefined,
    ...
    cardio_duration: cardio ? formatDuration(Number(cardio.duration_min)) : "",
    notes: entry.notes ?? "",
  };
}
